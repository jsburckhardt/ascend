...
`just proof-workbench-capacity` runs the designated-host BL-004 diagnostic once over fixed cohorts 1, 3, 5, and 10. It retains raw JSON plus a concise comparison under the Issue #11 implementation evidence directory. The three-member cohort is the only MVP gate; five and ten are findings. The strict 1,200,000 ms deadline surrounds setup, all cohorts, cleanup, and evidence retention. Both 5,000 ms windows sample at or after exact target offsets, workload stdout plus stderr share one 4,096-byte bound, and every process/listener identity discovered during later sampling remains cleanup-audited. Unexpected exits become failed slots; missing host or ready-tree evidence, probe/inspection failures, and incomplete evidence remain findings and can make the command nonzero. `just proof-workbench-capacity-audit` performs the short resource, evidence, and BL-001 fixture audit that also ends `just verify` without repeating the expensive episode.

The retained run `532abfdb-c970-4979-9da2-ec9ef99a295a` passed: all 19 requested members remained ready, all workloads passed, all scheduled host and ready-tree positions were retained, cleanup and fixture integrity passed, and the independent three-member gate passed. This is a diagnostic baseline, not runtime scheduling, quota, sleep, multi-host, BL-010, or BL-013 functionality. See [the workbench proof runbook](docs/workbench-proof.md) for exact prerequisites, bounds, sampling, evidence, results, and interpretation.
