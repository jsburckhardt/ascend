...
`GET /api/projects/runtime` returns `{"runtimes":[{"id":"stable-id","state":"Stopped|Starting|Running|Failed"}]}` in the same `createdAt ASC, id ASC` order as `GET /api/projects`; only a `Failed` row also has one of the 18 bounded `failureCategory` values. List or projection failure returns exactly `500 {"error":{"category":"runtime_state_failed"}}` and never a partial success. The existing `GET /api/projects` payload remains exactly the four persisted fields `id`, `name`, `canonicalPath`, and `createdAt`; registration and close contracts are unchanged.
