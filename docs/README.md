...
Project Home requests runtime state once per authoritative project-list revision, once per explicit Retry, once after each settled successful or already-stopped Stop, and once after each successful Restart. Exact ID-set and order reconciliation is required; loading, timeout, transport failure, and mismatch render whole-list unavailability rather than partial state. `Running` requires observed readiness. Start failure, post-readiness exit, failed health, false liveness, an unconfirmed stop, or a failed restart retain `Failed`; guarded transitions emit one terminal event with one category and bounded cleanup. No runtime state is persisted, and no polling, stream, or browser health probe is added.
...
Issue #41 adds Restart for authoritative `Running` and `Failed` workbenches through `POST /api/projects/{id}/runtime/restart`. The runtime manager is the sole replacement owner: it confirms the prior exact process, process group, and listener absent before launching one replacement generation. Eight concurrent same-project requests join one operation. Pending admissions and exact-identity quarantine prevent late launch materialization from being mistaken for absence, while different projects keep independent browser and manager lanes.

Restart projects publicly as `Starting`, emits only `runtime.restart.requested`, `runtime.restart.succeeded`, or `runtime.restart.failed`, and never exposes PID, port, authority, path, audit, or server text. The normal manager bound is 66,000 ms, admission/quarantine recovery is bounded by 81,000 ms, and the browser transport is 85,000 ms. Success performs one authoritative runtime projection and returns focus to the selected Restart action; rejection is retryable with client-owned text, and indeterminate transport remains explicitly unknown.

Use `just verify-runtime-restart`, `just proof-runtime-restart`, and `just proof-runtime-restart-residual-audit`. The deterministic matrix is retained under `project/work-items/41-bl-018-restart-a-running-or-failed-workbench/implementation/evidence/`; raw host identities and measured timings remain ignored under `test-results/bl-018/`. Restart adds no SQLite migration, persisted runtime handle, environment variable, daemon, or automatic recovery.
...
Native pickers, scanning, clone/import, repository detection, search, user sorting, tags, and path mutation remain deferred. Runtime status, selected Stop, and selected Restart are delivered by BL-016 through BL-018, and the close of a running or failed project by BL-020. Run root commands only: `just verify-open-project`, targeted `just verify-focused <test-path>`, and `just verify`.
...
Use just verify-close-project, targeted just verify-focused paths, and just verify. Generated evidence lives at test-results/bl-009/close-project/manifest-matrix.json, test-results/bl-008/open-project/episode.json, and test-results/bl-008/open-project/close-fault-episode.json. The close of a running or failed project is delivered by BL-020 and documented below; archive, undo, bulk close, and product cleanup are still not implemented. Runtime state, selected Stop, and selected Restart are separate delivered behaviors that preserve registration.
...
Issue #45 extends the same `DELETE /api/projects/{id}` request to a project whose workbench is `Running` or `Failed`. The runtime manager owns the whole operation: it claims the project, drains that project's proxy connections, resolves its admissions and quarantine, releases and audits its owned runtime, sweeps any remaining ownership record, re-observes zero residual, and only then removes the durable registration and retires the stable ID. A project with no owned runtime is still closed with no signal sent and no process or listener probed, so the delivered stopped-project behaviour is unchanged.

The close settles into eleven bounded results and nothing else: `closed`, `already-absent`, and the nine rejections `start-in-progress`, `stop-in-progress`, `restart-in-progress`, `reconcile-in-progress`, `reconcile-unresolved`, `release-unconfirmed`, `ownership-cardinality-exceeded`, `removal-failed`, and `manager-shutdown`. The route publishes eleven categories over twelve status rows: `200` for a completed close and `404 project_not_found` for an already-absent project are the two success statuses; `400 invalid_project_id`; `409 runtime_start_in_progress`, `runtime_stop_in_progress`, `runtime_restart_in_progress`, `runtime_reconcile_in_progress`, and `runtime_reconcile_unresolved`; `500 runtime_release_unconfirmed`, `runtime_close_ownership_unresolved`, and `project_close_failed`; and `503 runtime_manager_shutdown`. A `409` means the project is busy and the request is safe to repeat once that operation settles; a `500` means Ascend could not finish and states exactly which half was reached; a `503` means the API is shutting down. The success body remains exactly `{"id":"stable-id","disposition":"closed"}` and every failure body remains exactly `{"error":{"category":"<category>"}}`. Exactly one `project.closed` record is emitted per completed close, so eight concurrent closes of one project produce one `200`, seven `404`s, and one record. While a close is claimed, that project's Stop and Restart routes return the new `409 runtime_close_in_progress`, and peer projects are unaffected.

Close stays non-destructive to the filesystem and to peers. The confirmation dialog copy is unchanged, and a close removes only the metadata registration: no project directory, file, link, mode, or timestamp is ever written, and every executed outcome — success, unconfirmed release, removal failure, each admission rejection, the concurrent case, and the repeated case — is proven against complete recursive before/after manifests of the selected and peer fixtures. A rejected close leaves the registration and all four persisted fields exactly as they were.

Recovery is documented rather than implied. `release-unconfirmed` retains the registration and a `Failed` entry classified `close-release-unconfirmed`, and is resolved by retrying Close after the workbench settles or by letting API shutdown perform its one retained termination and audit. `removal-failed` means the runtime was released and only the registration remains, so a retry removes it. `ownership-cardinality-exceeded` means nothing was attempted; resolve the retained ownership with Stop or Restart first. An API interruption is safe at every point because durable removal is last, and the replacement API settles a surviving candidate through the unchanged one-shot reconciliation pass. See [project-runtime.md](project-runtime.md) for the admission order, the phases, the confirmation clauses, the bounds, and the recovery table, and [stable-workbench-routing.md](stable-workbench-routing.md) for the per-project drain and its two `503` rows.

Retained evidence is committed under `project/work-items/45-bl-020-close-a-running-or-failed-project/implementation/evidence/`: `close-matrix.json` with 75 executed scenario rows, `designated-episode.json` with the seven designated real-host episodes, and `residual-audit.json` with the independent nine-class re-observation. Working copies, host identities, measured timings, disposable fixtures, the browser episode, the component lane, and the filesystem non-mutation artifact `close-non-mutation.json` stay ignored under `test-results/bl-020/`. Every artifact carries opaque project tokens and bounded classifications only: raw canonical paths, argument vectors, executable or installation paths, process identifiers or start times, ports or loopback authorities, socket inodes, environment values, credentials, terminal or source content, stacks, and raw errors appear in neither public surfaces nor committed evidence. Bounded outcome, category, residual-class, and confirmation-clause names are recorded deliberately, so a claim that classifications are absent from committed evidence would be false. Each validation recipe removes exactly the resources it created — isolated databases and their `-wal`, `-shm`, and `-journal` sidecars, disposable fixtures, launched runtimes, control processes, and control listeners — after integrity capture, and the residual audit re-observes rather than trusting those claims.

Validate with `just verify-close-project`, which keeps its delivered name and now also runs the BL-020 unit, manager, route, evidence, matrix, component, and documentation suites; `just verify-runtime-close` for the full acceptance suite plus the browser proof; `just proof-runtime-close` for the designated real-host proof; `just proof-runtime-close-residual-audit` for the independent residual audit; targeted `just verify-focused <test-path>`; and `just verify` as the authoritative gate. Archive, undo, bulk close, product deletion, automatic lifecycle policy, and recovery beyond the delivered one-shot API-restart reconciliation are not implemented.
