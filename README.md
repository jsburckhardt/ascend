...
This delivery covers a registered project with no owned runtime. BL-017 provides a separate selected-runtime Stop and BL-018 a separate Restart, both of which preserve registration, and BL-020 extends the same Close request to a running or failed workbench as described in the next section. Archive, undo, bulk close, and product deletion are not implemented.


## Close a running or failed project (BL-020)

Close now accepts a project whose authoritative workbench state is `Running` or `Failed`, using the same `DELETE /api/projects/{stable-id}` request, the same one keyboard-focusable Close action per card, and the same confirmation copy. Every registered card keeps its Close action whatever its state; what BL-020 adds is that a `Running` or `Failed` project is no longer refused. Eligibility is never decided from the card's rendered text: the runtime manager re-evaluates the project itself, immediately before it claims. A project with no owned runtime keeps the delivered stopped-project path exactly — no signal is sent, no process, group, or listener is probed, and only the registration is removed.

The runtime manager owns the whole operation. It admits a close through a fixed nine-step order — shutdown, manager validity, persisted lookup, entry lookup, reconciliation pending, reconciliation unresolved, another lifecycle operation in flight, an existing close claim, and finally a synchronous claim — and every step before the claim settles without mutating anything. Once claimed, the close runs seven phases: it drains that project's proxy connections, quarantines and resolves pending admissions, releases the owned runtime through the delivered stop sequencer, audits release, sweeps any residual ownership record, re-observes zero residual, and only then commits the single durable removal and retires the stable ID. Ownership cardinality is frozen at claim time, the sweep runs `max(1, frozen)` units, and a project whose retained ownership exceeds the internal cap is refused before anything is attempted.

Exactly eleven bounded results exist: `closed`, `already-absent`, and the rejections `start-in-progress`, `stop-in-progress`, `restart-in-progress`, `reconcile-in-progress`, `reconcile-unresolved`, `release-unconfirmed`, `ownership-cardinality-exceeded`, `removal-failed`, and `manager-shutdown`. The route maps them onto eleven categories and their statuses:

| Result | Status | Category |
|---|---|---|
| completed close | 200 | success body `{"id":"stable-id","disposition":"closed"}` |
| malformed identifier | 400 | `invalid_project_id` |
| already absent, or a losing concurrent contender | 404 | `project_not_found` |
| a start is in flight | 409 | `runtime_start_in_progress` |
| a stop is in flight | 409 | `runtime_stop_in_progress` |
| a restart is in flight | 409 | `runtime_restart_in_progress` |
| reconciliation is pending | 409 | `runtime_reconcile_in_progress` |
| reconciliation is unresolved | 409 | `runtime_reconcile_unresolved` |
| the runtime release could not be confirmed | 500 | `runtime_release_unconfirmed` |
| retained ownership exceeds the internal cap | 500 | `runtime_close_ownership_unresolved` |
| the durable removal failed | 500 | `project_close_failed` |
| the API is shutting down | 503 | `runtime_manager_shutdown` |

A `409` means the project is busy and the same request is safe to repeat once that operation settles. A `500` states exactly which half was reached: `runtime_release_unconfirmed` and `runtime_close_ownership_unresolved` mean the registration is retained, while `project_close_failed` means the runtime was released and only the registration removal failed. A `503` means the API is shutting down. Failure bodies remain exactly `{"error":{"category":"<category>"}}`, and the success body is unchanged from BL-009. Eight concurrent closes of the same project produce one `200`, seven `404`s, and exactly one `project.closed` record. While a close is claimed, that project's Stop and Restart return the new `409 runtime_close_in_progress`, `start` and `register` fail with the acquisition-only `runtime-closing` failure, and peer projects keep independent lanes.

Close is non-destructive to the filesystem. The dialog copy is unchanged — Closing removes this project registration from Ascend. The filesystem directory and files will not be deleted. — and every executed outcome is proven against complete recursive before/after manifests of the selected and peer fixtures: membership, bytes, symlink targets, modes, and nanosecond timestamps. A rejected close leaves the registration and all four persisted fields byte-identical. `release-unconfirmed` retains both the registration and a `Failed` entry classified `close-release-unconfirmed`; retry Close after the workbench settles, or let API shutdown perform its one retained termination and audit. `removal-failed` means only the registration remains, so a retry removes it. `ownership-cardinality-exceeded` attempted nothing; resolve the retained ownership with Stop or Restart first. An API interruption is safe at every point because the durable removal is last: a survivor is settled by the unchanged one-shot reconciliation pass, and a repeated close after a committed removal is a bounded `404`.

Project Home issues at most one close per project, disables the dialog once the request is transmitted, keeps peer Open, Stop, Restart, and Close controls usable, and announces each transition with the project name. The client bound is 45,000 ms, chosen to strictly exceed the 41,000 ms caller-visible server ceiling so an ordinary slow server result is reported as itself rather than as an unknown outcome; an indeterminate transport result stays explicitly unknown until a Refresh resolves it. Close introduces no new environment variable, configuration default, deployment topology, process, port, service, or host requirement, and no data, schema, SQLite, or API-payload migration: the same four persisted project fields are stored, and no new value is persisted, so there is nothing to migrate.

Committed evidence lives under `project/work-items/45-bl-020-close-a-running-or-failed-project/implementation/evidence/`: `close-matrix.json` (75 executed scenario rows), `designated-episode.json` (the seven designated real-host episodes), and `residual-audit.json` (the independent nine-class re-observation). Host identities, measured timings, disposable fixtures, the keyboard-only Chromium episode, the component lane, and the filesystem non-mutation artifact `close-non-mutation.json` remain ignored under `test-results/bl-020/`. Public surfaces and committed evidence carry opaque project tokens and bounded classifications only — no raw canonical path, argument vector, executable or installation path, process identifier or start time, port or loopback authority, socket inode, environment value, credential, terminal or source content, stack, or raw error. Bounded outcome, category, residual-class, and confirmation-clause names are recorded on purpose, so committed evidence does contain classifications by design. Each recipe deletes exactly the resources it created — isolated databases with their `-wal`, `-shm`, and `-journal` sidecars, disposable fixtures, launched runtimes, control processes, and control listeners — and `just proof-runtime-close-residual-audit` re-observes rather than trusting those claims.

Run `just verify-close-project` for the extended deterministic gate, `just verify-runtime-close` for the full acceptance suite and browser proof, `just proof-runtime-close` for the designated real-host episodes, and `just proof-runtime-close-residual-audit` for the independent residual audit; `just verify-focused <test-path>` runs a targeted subset and `just verify` remains the authoritative gate. See [the runtime runbook](docs/project-runtime.md) for the admission order, phases, confirmation clauses, bounds, and recovery, [the routing runbook](docs/stable-workbench-routing.md) for the per-project drain, and [the proof runbook](docs/workbench-proof.md) for the designated episodes. Archive, undo, bulk close, product deletion, automatic lifecycle policy, and recovery beyond the delivered one-shot API-restart reconciliation are not implemented.
...
Run `just verify-runtime-reconcile`, `just proof-runtime-reconcile`, and `just proof-runtime-reconcile-residual-audit`. BL-020 Close consumes this pass unchanged: a close waits for no pass, refuses a `reconciling` project with `409 runtime_reconcile_in_progress` and a `reconcile-unconfirmed` project with `409 runtime_reconcile_unresolved`, and releases an adopted survivor through the ordinary sequencer. BL-021 automatic lifecycle policy and BL-022 durable or distributed recovery remain separate.
