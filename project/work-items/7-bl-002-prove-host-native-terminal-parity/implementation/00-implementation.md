...
- **AC-10:** README.md, apps/api/README.md, docs/README.md, docs/workbench-proof.md, .harness/engineering-harness.md, and the harness-change record document prerequisites, command list, PATH policy, normalization, paved command, bounds, diagnostics, evidence, cleanup, and designated-host result. The runbook now points to test-results/bl-001/terminal-parity/episode.json and states the exact coordinator-backed timeout cleanup proof.
- **AC-11:** just verify exited 0 after the Verify correction: format, lint, strict typecheck, 34 API tests and 1 web test with thresholds (80.92% API branch coverage), builds, and 3 Chromium E2E tests passed. The canonical gate passed one forced real timeout cleanup scenario and one passing parity episode.
