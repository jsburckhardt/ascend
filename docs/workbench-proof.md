...
The integrated command is dispatched once behind its lock and then observed within one finite evidence wait; no helper, Playwright, or recipe retry is used. Monotonic prerequisite/direct, workbench start, listener/HTTP readiness, browser readiness, terminal creation, command dispatch, command evidence, and cleanup phases have fixed evidence-derived bounds inside the 90,000 ms episode, including a 10,000 ms cleanup reserve. Proxy failure rows similarly retain execution-correlated project-token, transport, and classification events without clearing shared observations.
