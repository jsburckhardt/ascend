...
3. Add a `restartOwners = useRef(new Map<string, Owner>())` registry and a `restart(projectId: string)` controller method. Admission requires: `listStatus === 'success'`, `mode === 'editing'`, no `close` state for that project, no owner already registered for that project unless the existing state is `phase: 'retry'`, and no pending `stop` for that project. It must not call `invalidate()`, must not read or set `owner.current`, and must not bump the global `generation`.
...
13. Leave the Open, Stop, and Close controls' disabled expressions untouched so a pending restart never disables them.
...
### Documentation Impact
T-12 documents Home behaviour in `apps/web/README.md` and `README.md`.

### Expected Evidence
V-12 results plus matrix rows 13, 14, 15, 16, 42.
