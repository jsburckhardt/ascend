...
Run `just proof-runtime-close` for the designated real-host close proof and `just proof-runtime-close-residual-audit` for its independent audit. The proof builds the compiled API entry point first and then executes the `BL020_DESIGNATED=1` suite, so every claimed API generation runs the repository's compiled `apps/api/dist/server.js` with host-observed evidence: the observed argument vector equal to that entry, a listening socket attributed to that process, at least one served request, and an observed persistence file. A placeholder, in-process, synthesized, or assigned generation is rejected by name.

Seven episodes are executed and retained:

| Episode | What it executes | Retained result |
|---|---|---|
| E-1 | a running close with the exact release triple | `200` and disposition `closed`; the recorded root identity absent from the process table, the durable registration gone, the peer workbench and the subject project tree untouched, and the held proxied connection severed by the close itself |
| E-2 | a retained-failed close after a real unconfirmed release | `500` with `runtime_release_unconfirmed`; the project published `Failed` classified `close-release-unconfirmed`, the registration retained, and no `project.closed` emitted by any generation |
| E-3 | an adopted close after a real API-process restart | `200` and disposition `closed`; the adopted root identity absent from the process table, the durable registration gone, and the peer workbench and subject project tree untouched |
| E-4 | a controlled interruption landing between a host-confirmed release and the removal | the removal had not committed, so the registration survived; the replacement generation published `Stopped` because the release was already confirmed, and a safe Close retry settled `closed` |
| E-5 | a controlled interruption landing after the durable removal had committed | `200`, the workbench observed absent before the removal was observed; the replacement generation reconciles from the durable store alone, no candidate is attributable to the removed project, and it publishes no runtime for it |
| E-6 | a controlled interruption during release and sweep, strictly before confirmation, with a surviving attributable candidate | no durable removal, registration present on the replacement boot, zero signals delivered by the interrupted close; the boot adopted the survivor and published `Running`, and the safe Close retry then settled `200` |
| E-7 | one successful close followed by three sequential repeats | `200`, then three `404 project_not_found` repeats that produce no signal, event, runtime, or mutation; exactly one `project.closed`, and the project still absent after a real restart of the compiled server on the same durable store |

`E-6` records which branch of the unchanged reconciliation disjunction the replacement boot took and the conjunction element that decided it; `absent` and `Stopped` stay prohibited while a survivor is alive. Signals are attributed per episode phase — the interrupted close's, the replacement transition's own orderly cleanup, and the retry's — and are never aggregated into one claim. Each episode records its declared bound, its own elapsed value, its residual probes, and its teardown separately from the manager's own release audits.

The retained artifacts are `project/work-items/45-bl-020-close-a-running-or-failed-project/implementation/evidence/designated-episode.json` for the seven episodes and `residual-audit.json` for the independent post-capture re-observation of nine classes: validation-owned API processes, workbench processes, attributable descendants, listeners, proxy connections, timers, in-flight close operations, database sidecars, and disposable fixtures. That audit runs in a separate process, re-observes rather than copying any captured value, reports zero for every class only with a completed probe, and refuses an unfinalized, unclear, or malformed episode artifact by distinct named error. A connection is residual only while an endpoint still holds it: the audit ignores the listening socket, which the listener class re-observes on its own, and the kernel's post-close `TIME_WAIT` tuple, which lingers for twice the maximum segment lifetime after both endpoints completed their FIN exchange and which no socket, and therefore no process, owns. Every other non-listening state is counted, so an established, half-closed, or unclosed connection still fails the audit. Host identities, timings, disposable fixtures, and the browser episode stay ignored under `test-results/bl-020/`. The keyboard-only Chromium episode proves the dialog flow for a `Running` card and a `Failed` card, peer-control availability during a pending close, the announcements, focus recovery, the unknown-outcome resolution, a pre-close WebSocket that can no longer exchange frames, and the post-close stable-route error.
