...
- T-2: Added ordered browser-event capture, semantic/status-based blocking precedence, successful-resource regression coverage, warning counts, repeat retention, and WebSocket usability.
...
- AC-8: browser-event tests prove both observed HTTP 200 md-math-block/code-server URLs are non-blocking and cover every policy family, overlap, precedence, and repeats. The regenerated comparison retains embedded totals 5/27 and full-page totals 3/33 for blocking/non-blocking occurrences.
...
- AC-20: selector matrix isolates all three tie-breakers. The regenerated result selected full-page at the first strict difference, 3 blocking versus 5.
...
- AC-27: README.md, docs/README.md, docs/workbench-proof.md, .harness/engineering-harness.md, the regenerated Accepted ADR, and decision log match comparison b1000003-0000-4000-8000-000000000009, including corrected 5/27 and 3/33 totals and retained raw-artifact requirements.
- AC-28: the final just verify run exited zero; formatting, lint, type checks, 84 unit/component tests with coverage, builds, and Playwright completed with 3 passed and the designated-only comparison test skipped by the ordinary gate.
...
- Corrected focused run: just verify-focused apps/api/test/workbench-presentation-*.test.ts, 8 files and 49 tests passed.
...
- Full gate: earlier just verify runs exposed formatting and one unused-variable defect; after correction, the final just verify exited zero. Unit/component tests reported 83 API plus 1 web pass, and Playwright reported 3 passed with 1 designated-only skip in 38.0 seconds.
