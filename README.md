...
Development Vite forwards same-origin `/projects` HTTP and WebSocket traffic to the API. Deployment front doors must preserve that same path ownership. No new environment variable, API contract, database field, or migration is introduced. Run `just verify-home-workbench` for component/API matrices plus the two real Chromium scenarios, `just proof-home-workbench-residual-audit` for independent cleanup evidence, and `just verify` for the full gate. See [the stable-routing runbook](docs/stable-workbench-routing.md).
