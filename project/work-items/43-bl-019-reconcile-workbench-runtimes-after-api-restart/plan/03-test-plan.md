...
**What changed from revision 4.** Three repairs, no renumbering, and no count change anywhere: **20** validations, **66** scenarios and their bounds and ACs, **18** refusal reasons, **20** source guards, **12** mutation classes, **13** sources, **13** declared bounds, **18** episode phases, **15** episode rejection reasons, and **22** AC IDs all stand.

1. **Check 9's membership conjunct (`V-2`, `V-6`, `V-15`, `V-16`).** A completed process-group enumeration that omits the candidate leader must refuse `group-scan-incomplete` before any listening-socket lookup. `V-2` now names all three cases at the helper layer, `V-6` proves the manager-level consequence that no listener, descriptor, or readiness observation follows the refusal, `V-15` exercises the extended `reconcile-listener-group-scoped` guard and the extended `M-9` fixture, and `S-17` moves from the completeness input to the membership input while keeping its identifier, bound, refusal class, and ACs. The completeness branch keeps its own deterministic coverage at `V-2` and `V-6`, so both branches remain separately provable without a sixty-seventh row.
2. **Trusted readiness pacing (`V-7`, `V-15`, `V-16`).** No reconciliation path may await `processDependencies.sleep`; every poll gap is armed from the injected `RuntimeDeadlineScheduler`, clamped inside `reconcileReadinessBoundMs`, and abandoned on abort. `V-7` proves the arming, the clamp, the cancellation, and the inertness of a late callback; `V-15` exercises the extended `reconcile-deadline-trusted-scheduler` guard with its two negative controls; and both `V-7` and `V-16` inject a `sleep` that **fails when invoked**, so a reconciliation that depended on the fallible primitive could not settle. No real sleep and no retry enters any deterministic proof.
3. **The two-tier disclosure boundary (`V-19`).** Bounded refusal-reason enum names legitimately appear in trusted inspection and in retained validation evidence, the committed matrix included; raw host values appear in neither tier. `V-19` now asserts that the documentation states exactly that and agrees with the committed artifact, instead of asserting a sentence the artifact contradicts.
...
6. `resolveGroupListenerOwner`: returns `group-scan-incomplete` for **each of these three inputs, asserted separately (revision 5)** — an enumeration reporting `complete: false`; a `complete: true` enumeration whose `pids` is empty; and a `complete: true` enumeration whose `pids` is non-empty but omits the candidate leader — and in each case asserts through injected spies that `readLoopbackListenerInode` and `readProcessSocketInodes` were called **zero** times, so the refusal precedes the listener lookup. It further returns `listener-absent` when no loopback LISTEN row exists; `listener-not-owned` for an inode held outside the group, held by a member with a foreign `argv[0]`, held by a member whose `argv[1]` escapes the installation root, held by an unobservable holder, and held by two observed members; `{ owner }` when exactly one conforming member holds it, once with the leader as owner and once with a forked member as owner.
...
11. **Revision 5 — check 9 at the manager layer.** With one otherwise conforming candidate, inject `readProcessGroupMemberPids` returning `{ complete: true, pids: [<forked member only>] }` and assert the project settles `unresolved` / `group-scan-incomplete`, that the injected ledger records zero `readLoopbackListenerInode`, zero `readProcessSocketInodes`, and zero health probes for it, that `listenerAttributed` is `0` and `listenerOwner` is `null`, and that `launches` and `signalsSent` are `0`. Repeat with `{ complete: true, pids: [] }` and with `{ complete: false, pids: [<leader>] }` and assert the identical settlement and the identical zero-call ledger, so the two sub-branches are proven separately and named identically.
...
2. Assert `processDependencies.sleep` is never called on any reconciliation path and that `deadlineScheduler.now()` is the only time source. **Revision 5, proven by construction rather than by a spy alone:** inject a `sleep` implementation that fails when invoked, run a readiness-polling scenario to settlement, and assert it settles exactly as catalogued — a reconciliation that depended on the fallible primitive could not.
2b. **Revision 5 — the poll gap is trusted, clamped, cancellable, and inert.** Assert that each gap between two health attempts is armed through the injected `deadlineScheduler.scheduleDeadline` for at most `config.pollIntervalMs` and never for more than the time remaining to `readinessDeadlineAt`; that advancing the injected clock by exactly one gap releases exactly one further health attempt; that aborting the readiness controller mid-gap resolves the gap, exits the loop, issues no further health probe, and settles the project by its enclosing reason rather than from an unread verdict; that the delay never rejects; and that a scheduler callback delivered after cancellation resolves nothing, records nothing, and mutates no entry. Assert the gap arms no `setTimeout`, `setInterval`, or timer module directly.
...
### Expected Evidence
Deadline arming and cancellation traces, the sleep-call spy at zero and the failing-`sleep` control settling normally, the per-gap scheduler arming and clamp trace, and the 13-entry bound registry.
...
### Expected Evidence
A report listing 20 guards passing with 20 negative controls — including the four revision-5 negative fixtures for the two extended guards — 12 mutation classes rejected, 4 episode artifacts classified, all **15** episode rejection reasons exercised (the 3 revision-2 status rejections in step 7 and the 12 revision-3 rejections in step 8), and the privacy-scanner derivation proof.
...
**Revision 5.** Every scenario injects a `sleep` implementation that **fails when invoked**, so no row can depend on the fallible primitive; readiness pacing is driven entirely by the injected `RuntimeDeadlineScheduler`, which keeps the polling rows — `S-21` above all — deterministic on injected time with no real sleep and no retry. `S-17`'s injected `readProcessGroupMemberPids` returns `{ complete: true, pids: [<one forked member>] }`, omitting the candidate leader, so the row records the membership sub-branch of check 9 and its ledger shows zero listener, descriptor, and readiness observations for that project.
...
| S-17 | Process-group enumeration completes without the candidate leader | 1 | unresolved | `group-scan-incomplete` | 11,000 | AC-10, AC-12 |
...
8. **Revision 5 — the two-tier disclosure boundary.** Assert `docs/api-restart-reconciliation.md` no longer states that refusal reasons never enter a committed artifact; that it states instead that public surfaces — browser-visible surfaces, HTTP bodies, and lifecycle events — carry only the opaque project token, the four public states, and the bounded public categories, while the bounded internal classifications exist in trusted in-process inspection and in retained validation evidence including the committed matrix; and that the raw-value list is excluded from **both** tiers. Cross-check the claim against the artifact itself: read the committed `runtime-reconcile-matrix.json`, require at least one project carrying a `refusalReason` drawn from `RECONCILE_REFUSAL_REASONS`, and require zero matches for the protected raw-value patterns, so the documentation and the evidence agree by observation rather than by assertion.
9. **Revision 5 — the two behavioural corrections.** Assert the document states that a workbench is adopted only while the bounded enumeration of its own process group both completes and contains that workbench's leader, and that recovery paces its readiness checks on the same trusted monotonic scheduling as its deadlines so every wait is bounded and cancellable. Assert neither statement names a primitive, helper, host path, pid, port, or interval value.
...
| Both `group-scan-incomplete` sub-branches proven separately, and refused before any listener lookup | S-17 (membership) and V-6 (both) at the boundary, V-2 (all three inputs) at the helper, V-15 (extended `reconcile-listener-group-scoped`, extended `M-9`) |
| No reconciliation path awaits a fallible delay primitive, and every poll gap is trusted, clamped, and cancellable | V-7, V-15 (extended `reconcile-deadline-trusted-scheduler`), V-16 (failing-`sleep` control on every row) |
| The documented disclosure boundary agrees with the committed artifact in both tiers | V-19 |
