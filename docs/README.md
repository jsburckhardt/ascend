...
The recursive BL-009 matrix compares relative membership, type, bytes, symlink target, mode, and nanosecond mtime after Cancel, success, unknown ID, rollback fault, ambiguity, same-ID retry, repeated absent close, and exact eight-way concurrency before cleanup. The desktop Chromium flow performs keyboard registration, Cancel, Confirm, authoritative-list removal, and unchanged sentinel validation. A separate repository-controlled one-shot persistence fault proves safe Retry and recovery. Cleanup removes only owned process groups, listeners, isolated database plus -wal/-shm/-journal, and disposable fixtures after integrity capture.
