...
The real process test launches A/B/C through direct argv process creation and checks each retained final canonical argument, cwd, OS user, complete environment allowlist, and absence of peer fixture values. The schema-version-2 fake artifact has 12 independently executed scenarios and actual scenario-owned lifecycle/proxy events. It measures the 24-call schedule; malformed/unknown/closed immutability while A/B/C run; five B-only faults with A/C terminal checks; one/all cancellation; replacement; and independent global-shutdown and shutdown-race episodes. Global shutdown records A graceful, B escalated, and C cancelled logical outcomes, exact physical audits, during/after typed rejection, unrelated-control survival, and no stale entries/background work.

All 24 ordered cross-target rows execute through an HTTP request or WebSocket upgrade boundary with unique execution and request/upgrade IDs. Each observes the declared mismatch, zero nonmatching contact, and a measured proxy/socket cleanup audit. Protected path, authority, environment, Git, terminal, source, and sentinel values traverse real launch, request/frame, event, and access/application-log paths. The scanner derives match arrays from the bytes of five fake evidence sources and the Chromium public artifact; artifacts that assign a zero, omit a source, copy events, or omit execution/audit IDs are rejected.

The no-retry Chromium episode uses three disposable Git repositories and an actual disposable SQLite library. It captures one Management and one ExtensionHost role for each initial project. After exact B termination, A and C execute terminal commands again with unchanged identities/routes; one explicit B replacement opens one fresh Management and ExtensionHost pair and records the replacement identity, route, token, and lifecycle event. Public evidence contains only safe digests/classes; the sole ignored mode-0600 restricted artifact retains exact authority for independent cleanup.
