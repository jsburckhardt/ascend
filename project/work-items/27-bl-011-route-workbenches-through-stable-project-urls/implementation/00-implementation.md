...
- T-0: Preserved linked-worktree repository validation and cancellation-versus-early-exit classification with three concurrent cancellation regressions; replaced renderer-specific terminal readiness waits with a bounded idempotent shell handshake, explicitly focused the terminal input, bounded cancellation PID observation within a 40-second overall episode window, resolved proof commands and evidence paths from each owned worktree root, and gave process-absence observation a finite 5-second load margin.
