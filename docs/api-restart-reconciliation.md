...
Refusal reasons, identities, ports, argv, installation paths, socket inodes, environment values, and raw errors remain trusted in-process detail. They never enter a browser-visible surface, HTTP body, or committed artifact. Public acquisition exposes only the fixed 503 `workbench_reconcile_unconfirmed` failure, and Stop or Restart exposes only bounded route categories.
