...
- **T-3 — Extend the single BL-001 Chromium episode (AC-1–AC-6, AC-8, AC-9).** Reuse the existing launcher, fixture, handle, process audit, and browser sensor; explicitly own one browser context, open one terminal, run the integrated helper, compare results, and retain Explorer/Markdown coverage.
