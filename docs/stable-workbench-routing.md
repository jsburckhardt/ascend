...
For simultaneous A/B/C traffic, every target is resolved from the matching immutable runtime snapshot. Before opening an HTTP stream or WebSocket, the proxy verifies stable project ID, canonical path, stable route, opaque owner token, loopback URL, and port against persisted metadata and the parsed route, then requires `ProjectRuntimeManager.ownsSnapshot` to prove the returned object is still the exact running entry. A forged snapshot copy cannot substitute another project’s otherwise valid target. Pending operations, streams, handshakes, raw sockets, and WebSockets remain tagged by project token and are audited per project and in aggregate.

The finite matrix executes all six ordered A/B/C pairs for project/route, HTTP target, WebSocket target, and frame-destination mismatch: 24 rows. HTTP rows cross the stable request boundary and WebSocket rows cross the upgrade boundary. Every row has unique execution and request/upgrade IDs, the selected opaque token and route class, exact `workbench_runtime_project_mismatch` outcome, measured zero nonmatching upstream contact, and a post-row proxy/socket cleanup audit. Malformed, unknown, and closed IDs execute while all three listeners remain live and leave snapshot/process/listener/route/project-event and terminal-sentinel measurements unchanged.

Five separate B-only episodes execute early exit, post-running crash, readiness failure, failed health reuse, and upstream-reset proxy failure through manager/process/proxy boundaries. A/C exact identity, route, listener, terminal health, and sentinel digests are checked after each. Cancellation and shutdown episodes own separate events rather than reusing concurrency observations. Event validators require unique execution/event IDs, matching opaque tokens, allowed state/transport/elapsed/classification fields, and nonempty measured audits.

Protected values travel through actual launch, HTTP, WebSocket, event, access/application-log, public-artifact, and restricted-artifact paths. Scanners derive literal and encoded match arrays from source bytes; declared or assigned zero constants do not satisfy the evidence contract. Public evidence contains safe correlations only. One ignored owner-readable mode-0600 restricted authority artifact supports independent exact-identity cleanup.

Run `just verify-project-runtime-isolation` for the schema-version-2 executable matrix and the three-project Chromium episode, then `just proof-project-runtime-isolation-residual-audit` independently. Chromium records exact Management and ExtensionHost role counts per project, repeats A/C terminal commands after B termination, and correlates B’s replacement identity/event/route. The residual command independently probes runtime identities/listeners, database/sidecars, and fixtures and validates measured browser/proxy/process-group/socket/terminal/background inventories. It proves only immediate isolation while all three runtimes are active; BL-014 switching/session persistence, BL-015 performance, and public Stop/Restart remain excluded.
