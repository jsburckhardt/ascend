...
- **T-4:** Added the read-only contract matrix, local command-boundary proof, lifecycle sensor, and 25 deterministic negative fixtures, including real malformed process/tool source.
...
- **AC-18:** `test-results/issue-23/rpiv-harness-contract-matrix.json` contains 113 deterministic rows and 38 rules across APS, five RPIV targets, documentation, and executable regressions, with zero failures; target hashes are unchanged by validation.
- **AC-19:** Twenty-five fixtures under `tests/contracts/fixtures/rpiv-harness/` each fail for exactly its expected rule, covering omitted, extra, and type-mismatched RUN arguments; invalid slash and unregistered tools; malformed hook parameters; serialization gaps; and the prior hook, correction, worker, observation, output, retention, overlap, and profile defects. The inspectable report is `test-results/issue-23/rpiv-harness-negative-fixtures.json`.
...
- `README.md` documents the registered `vscode/runCommand` host boundary, exact `eng-harness-flow` argument semantics, 25-fixture inventory, executable contracts, 113-row matrix, and `just verify-rpiv-harness` workflow.
...
- `just verify-rpiv-harness` — repeated passes; 113 matrix rows, 38 rules, zero failures, 25 expected negative-fixture failures.
- `just verify` — exit 0; format, lint, typecheck, 308 API tests, 139 web tests, 5 RPIV contract tests, 48 registration tests, builds, 5 passed/1 skipped Playwright tests, and capacity audit passed.

## Observation Evidence

- Captured verifier-feedback backtracking as `COORD-039`.
- Captured unavailable inspection tools and targeted-edit retries as `DL-231`, `DL-232`, `DL-233`, and `DL-235`.
- Captured focused/full validation failures before correction as `DL-234`, `DL-236`, `DL-237`, and `DL-238`.
- Captured the full-gate wait exceeding 30 seconds as `INS-041`.
