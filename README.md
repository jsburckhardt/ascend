...
The schema-version-2 fake evidence contains 12 independently executed scenarios: interleaved starts; malformed, unknown, and closed IDs while A/B/C remain live; five distinct B-only Early exit, crash, readiness, health, and proxy faults; all-caller and one-caller cancellation; explicit replacement; global shutdown; and the separate shutdown race. Scenario events carry their own execution/event IDs, project tokens, states or transports, elapsed values, and classifications. All 24 ordered project-route, HTTP-target, WebSocket-target, and frame-destination mismatch rows cross the proxy request or upgrade boundary, bind a request/upgrade ID, observe `workbench_runtime_project_mismatch`, prove zero nonmatching upstream contact, and measure cleanup.

One cancelled B waiter does not cancel the seven callers sharing B’s running result; cancelling all eight cleans only B. After every B fault, A/C identity, route, listener, and terminal-sentinel checks remain unchanged. Global shutdown records exact graceful, escalated, and cancelled project outcomes, joins repeated shutdown, rejects starts during and after shutdown, settles late work, and leaves an unrelated control alive until its separate cleanup. The repository-only test authority is unavailable to routes, measured per-project/global cleanup is mandatory, and no public Stop or Restart endpoint or UI was added.

Run `just verify-project-runtime-isolation` for the fake matrix and no-retry three-Git-fixture Chromium A/B/C episode. After repository-only B termination, A/C terminal commands execute again; a fresh B page records one Management and one ExtensionHost role for the replacement while A/C retain their original role sets. Public evidence stores correlated identity/route/event digests. One ignored mode-0600 restricted artifact retains exact cleanup authority. `just proof-project-runtime-isolation-residual-audit` independently probes initial/replacement process identities, listeners, SQLite files/sidecars, fixtures, and the control, and validates measured browser, proxy, runtime/process-group, socket, terminal, and background-work inventories. Assigned zero rows and unexecuted scans fail. Commands use repository-local fixtures, finite bounds, no hosted service, credential, network dependency, or manual judgment.
