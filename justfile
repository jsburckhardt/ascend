...
verify-home-workbench api_build="true":
    ...
    if [[ {{quote(api_build)}} == true ]]; then pnpm --filter @ascend/api build:ts; fi
...
verify-session-switching:
    BL014_ACCEPTANCE=1 pnpm exec vitest run apps/api/test/session-switching-contract.test.ts apps/api/test/session-switching-matrix.test.ts apps/api/test/session-switching-residual-audit.test.ts apps/web/src/session-switching-component.test.tsx --reporter=verbose
