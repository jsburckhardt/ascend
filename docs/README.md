...
Development Vite and deployed front doors must forward `/projects` HTTP and WebSocket traffic to the API on the Home origin. There is no new application environment variable, API payload, database field, or migration. Run `just verify-home-workbench`, `just proof-home-workbench-residual-audit`, and `just verify`. See [stable-workbench-routing.md](stable-workbench-routing.md) for route behavior, exact recovery, privacy, and operations.
## Source Documents
