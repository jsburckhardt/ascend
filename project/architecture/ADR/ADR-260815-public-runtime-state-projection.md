...
- Amended 2026-08-16 (Plan revision 2 restates this paragraph; revision 1 wrongly listed `runtime-closing` as a projected `Failed` category): the BL-020 portion of that deferral is spent by [ADR-260816-selected-project-close-control](./ADR-260816-selected-project-close-control.md), which removes one selected registered project whose runtime is `Running` or a current-process retained `Failed`, including an adopted runtime. **No fifth public state, no additional internal entry state, and no change to the one-synchronous-pass projection or the no-polling rule is introduced.** A close claim is orthogonal to the entry it claims, so a claimed `running` entry keeps projecting `Running` and a claimed retained `failed` entry keeps projecting `Failed`; a completed close removes the durable registration, so the project has no public runtime row at all rather than a synthetic terminal state. Two bounded failure categories are added, and only one of them is ever projected. `close-release-unconfirmed` is the **sole new retained classification**: a close that could not confirm release installs it on a `failed` entry, and the projection reports that entry as `Failed` through the existing `failureCategory` field, exactly as it already does for every other retained category. `runtime-closing` is **not a projected state at all**: it is an acquisition failure that `start()` and `register()` raise when they meet an installed close claim, it is never written into an installed `failed` entry, and it is therefore never read by this projection. It surfaces only where an acquisition failure already surfaces — the workbench failure table's route mapping and the client's notice vocabulary — and any implementation that installs it as an entry failure, or any document that describes a project as reporting `Failed` because a close is in flight, contradicts this decision. A claimed project keeps projecting the entry it already holds. A confirmed release whose durable removal then failed installs a released `registered` entry and is therefore reported `Stopped`, never `Running` and never `Failed`. BL-021 automatic lifecycle policy, BL-022 durable or distributed recovery, automatic recovery beyond one reconciliation, persisted runtime state, background health monitoring, extra public states, and dashboards remain deferred.
