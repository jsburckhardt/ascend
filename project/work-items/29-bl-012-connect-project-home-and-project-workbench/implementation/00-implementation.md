...
## Verifier readiness correction at 4071127

- **AC-24:** `waitForProcessHttpReady` now treats API and Vite output as a nullable timestamped hint only. It repeatedly requires strict inode/port attribution to the exact launched process tree and then the expected HTTP status/body from that same reserved endpoint. It rejects process exit, cancellation, unexpected responses, unrelated listeners, and the finite per-step timeout. Both continuity artifacts retain distinct `logHintAtMs` and `listenerReadyAtMs` fields plus the named HTTP consequence; the residual audit requires all four API/web records.
- **AC-28:** the deterministic fake emits `Local:` and waits on a parent barrier before it starts listening, proving no premature readiness. Additional rows cover never-listening timeout, early exit, cancellation, and exact fake-process cleanup. Vite IPC now reports only `log-hint`; no log or bare socket event can report ready.
- Final no-retry standalone gates passed three consecutive times with immediate residual audits. Wall durations were 136, 127, and 132 seconds. Real-process/continuity durations were 57,733/54,616 ms, 55,539/46,956 ms, and 56,733/49,305 ms; maximum retained steps were 19,176/20,181 ms, 18,114/19,796 ms, and 18,792/20,450 ms. Every residual audit reported continuity, real-process, readiness, and cleanup true.
- `just verify-focused`: PASS, 84 files passed with 1 skipped; 572 tests passed with 2 skipped. `just verify`: PASS in 293 seconds. The first `harness boot` attempt exposed a BL-011 residual evidence test timeout under full-suite load; its focused rerun passed 4 tests in 6.01 seconds. Final `harness boot`: PASS with readiness `ready`, checks duration 300,623 ms inside 610,000 ms, and 307-second wall duration.
- Documentation changed: `README.md`, `docs/stable-workbench-routing.md`, the T-6/T-8 correction record, and V-5/V-6/V-9/V-10 correction validation now distinguish log hints from exact listener-and-HTTP readiness and describe the regression matrix. No product API, configuration, schema, data, migration, deployment topology, ADR, or core-component contract changed.
- This is implementation evidence only. Final acceptance remains owned by Verify.
