...
- **AC-1:** just proof-terminal-parity exited 0; one existing BL-001 workbench and one terminal creation action ran against the canonical fixture. The episode records a 90,000 ms overall timeout and completed in 18,249 ms. Overall-timeout tests return terminal-episode-timeout nonzero.
...
- **AC-8:** Passing episode cleanup records browserContextClosed=true, terminalCommandsAbsent=true, exactHandleAbsent=true, listenerAbsent=true, firstStopExit=0, and repeatedStopExit=0. Negative cleanup matrix covers timeout, artifact/post-browser, cancellation, context close, stop, and audit errors without broad kills.
...
- **AC-10:** README.md, apps/api/README.md, docs/README.md, docs/workbench-proof.md, .harness/engineering-harness.md, and the harness-change record document prerequisites, command list, PATH policy, normalization, paved command, bounds, diagnostics, evidence, cleanup, and designated-host result.
- **AC-11:** just verify exited 0: format, lint, strict typecheck, 33 API tests and 1 web test with thresholds (80.52% API branch coverage), builds, and 2 Chromium E2E tests passed; terminal parity episode passed once in the canonical gate.
