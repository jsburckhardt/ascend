...
This component applies to the Fastify workbench route, the top-level browser navigation shell, raw HTTP forwarding, Node upgrade handling, WebSocket bridging, project and runtime resolution, proxy socket ownership, safe failures, structured events, local evidence, shutdown, and route documentation. It defines immediate multi-project transport isolation but does not add scheduling, quotas, session-switching continuity, public networking, authentication, TLS, multi-host operation, alternate editors, runtime-status UI, or user lifecycle controls.

## Definition

### Rules
- Front-door authority metadata MUST be accepted only when x-ascend-front-door-authority and x-ascend-front-door-token form a complete valid pair authenticated by the private ASCEND_FRONT_DOOR_TOKEN. The local unset default is development-only; explicit values MUST contain 16–256 characters and align across proxy and API. Partial, malformed, or mismatched trusted headers MUST fail before project lookup or runtime start and MUST NOT fall back to socket authority. Tokens and trusted headers MUST be absent from browser, response, log, and retained evidence surfaces.
- Ascend MUST own `/projects/{projectId}/workbench/` and every descendant for HTTP and WebSocket traffic. The browser URL segment MUST contain the percent-encoded persisted stable project ID, not the opaque owner token. The decoded project ID MUST be one 1-to-128-character segment matching `^[A-Za-z0-9][A-Za-z0-9._~-]{0,127}$`; the close-project API retains its separate opaque-ID contract.
- An unmarked top-level browser document request for the valid stable base route MUST receive an Ascend-owned navigation shell before upstream acquisition. The shell MUST keep the stable URL unchanged, acquire the upstream document through that same URL with a server-recognized marker that cannot select a target, and keep marked acquisition, descendant HTTP, and WebSocket traffic under the existing proxy contract. Malformed routes MUST render the shell error before project lookup or runtime start.
- The proxy MUST resolve the project with `ProjectLibrary.findById`, start or reuse it only through `ProjectRuntimeManager.start`, and derive the upstream solely from the immutable running snapshot. Before forwarding, it MUST verify that the snapshot project ID, canonical path, stable route, and opaque owner token match the persisted project and parsed route, that the loopback URL matches the snapshot port, and that the runtime manager confirms this is the exact immutable object still installed as that project’s running entry; any mismatch or forged copy MUST fail closed without reaching an upstream. Client host, forwarding, proxy-target, port, canonical-path, and authority values MUST NOT select or modify the upstream.
...
- The proxy MUST NOT coordinate, initiate, join, delay, drain, or reconnect an explicit runtime restart. A proxy-path start that reaches a project whose entry is `restarting` MUST receive the bounded restart-in-progress runtime failure and MUST surface it as a bounded failure rather than waiting, retrying, or forwarding to a prior-generation upstream. That bounded failure MUST be a published `WORKBENCH_FAILURE_TABLE` row, `runtime:runtime-restarting` at `503` `workbench_restarting`. More generally, `WORKBENCH_FAILURE_TABLE` MUST be exhaustive over the whole proxied runtime category union: every runtime failure category except manager shutdown MUST have exactly one row, in the declared runtime-category order, because the lookup throws for an unlisted category and would otherwise convert a bounded lifecycle state into an uncategorised `500`. A retained failure surfaced by a reuse-ownership recheck and an in-flight-operation failure are both proxy-reachable and MUST have rows, as is a bounded failure retained by a restart whose replacement cleanup could not confirm absence, which MUST be published as a `503` because a later explicit action can clear it. That exhaustiveness MUST be enforced mechanically — the table's own category union and the declared failure-category union MUST each be provably free of members the other lacks — so no future runtime category can be introduced without a published row, a status, a code, and a safe message.
- A prior-generation running snapshot MUST fail the exact-snapshot ownership check as soon as a restart claims its generation, so no request or WebSocket may be forwarded to a released upstream. Prior-generation browser and proxy connections MUST be left to terminate with the released runtime's confirmed-absent process and listener; the proxy MUST NOT be given a broadcast, drain, or forced-close restart path.
- The stable `/projects/{projectId}/workbench/` route MUST remain unchanged across a replacement, and a fresh navigation through it MUST resolve the successor snapshot through the ordinary acquisition path. No surface may claim that editor, terminal, or other runtime-scoped session state survives a replacement.
...
- Browser-visible URLs, downstream headers, downstream bodies, redirects, and cookies MUST contain no selected loopback authority. HTTP forwarding MUST request identity encoding and MUST rewrite a selected authority across textual response-header values and streamed body chunk boundaries to the stable route without buffering the complete body. Every unencoded textual response MUST remove upstream `Content-Length` before this transform and use streamed framing because replacement can change the byte count; byte-identical binary or encoded responses MAY preserve the original length. Proxy and access events MUST contain only stable names, the same deterministic opaque project token carried by the selected runtime snapshot and lifecycle events, safe classifications, elapsed values, and bounded counts. That one-way token MAY appear in declared privacy-safe public evidence for correlation. Events and public evidence MUST exclude raw project IDs outside the encoded stable URL, raw canonical paths, internal ports and authorities, authorization, cookies, credentials, secrets, query or body values, command or environment data, and HTTP, WebSocket, or terminal payloads. Restricted mode-0600 evidence MAY contain exact authority details only where its evidence contract explicitly allows them.
...
- Three fresh workbench workflows MUST open exactly six VS Code remote-protocol WebSockets: one Management role (`desiredConnectionType=1`) and one ExtensionHost role (`desiredConnectionType=2`) per workflow. All six MUST be same-origin under the stable prefix with `reconnection=false`. Unknown, duplicate, missing, retry, external-origin, or internal-port socket observations MUST fail. Evidence MUST retain only role and safe URL shape, not handshake payloads or reconnection tokens. The BL-012 navigation proof MUST additionally classify any observed Tunnel (`desiredConnectionType=3`) control role and any socket cancelled before a control payload; unsupported protocol roles remain failures.
- WebSocket bridging MUST use finite precommit handshakes, preserve text and binary message bytes and order, propagate ping and pong behavior, preserve clean close codes and reasons, convert an upstream abnormal termination into downstream abnormal termination without transmitting reserved codes `1005` or `1006`, and apply backpressure before accepting more frames. A disconnected downstream MUST cancel only its own pending handshake or upstream peer.
- The proxy owner MUST inventory pending requests, upstream HTTP streams, handshakes, raw sockets, and upgraded WebSockets in memory with their opaque project token and MUST expose both per-project and aggregate audits. Shutdown MUST reject new work, return the manager-shutdown failure before commitment, close committed streams and upgrades without a second status, settle proxy work within 5,000 ms, and complete before the runtime-manager and persistence shutdown steps. Runtime processes remain exclusively owned by BL-010.
- Upstream HTTP response headers and WebSocket handshakes MUST each have a 5,000 ms timeout. Test-only shorter injected bounds MAY be used. Every marked document-acquisition, descendant HTTP, or WebSocket precommit failure MUST use exactly `{"error":{"code":"<code>","message":"<message>"}}` and the following non-overlapping table. The top-level navigation shell MUST translate only these safe outcomes, plus its bounded document-load timeout, into fixed accessible error content at the unchanged URL; Retry MUST create one newer shell generation without adding history, and Projects MUST navigate to `/` without starting a runtime:
...
- Executable security validation MUST enable and marker-bound both access and application logs. HTTP secrets MUST travel through their HTTP request fields, WebSocket payloads through real frames, and integrated-terminal payloads through distinct terminal WebSocket frames; disabled logging or header substitutes MUST fail. Every failure-table row MUST execute through the stable route and its controlled library, runtime-manager, proxy-manager, or fake-upstream boundary with an execution ID, observed internal category, public mapping, redaction result, and zero-resource cleanup; local throws and table-only comparison MUST fail.
...
- **Amended 2026-08-15 (revision 2).** Acquisition of an already-adopted runtime is the primary on-demand correction path for an adopted runtime that has since died: it MUST perform the delivered liveness and readiness re-checks before reusing that runtime, and MUST surface the delivered failure rather than routing to a dead identity. The reconciliation bound it awaits is the internal reconciliation allowance, which is strictly smaller than any externally declared end-to-end ceiling measured from the API process start.
- Workbench acquisition for a project whose API-restart reconciliation has not settled MUST await that project's settlement inside the declared reconciliation bound and then follow the settled outcome: reuse the adopted runtime, perform an ordinary start for a positively absent runtime, or surface the bounded reconciliation failure. It MUST NOT launch a runtime while that project's reconciliation is pending or unresolved, and MUST NOT start a reconciliation of its own.
- Concurrent acquisitions for one project held across a reconciliation boundary MUST join one settlement and one subsequent outcome, MUST reuse the same adopted identity and listener when adoption succeeded, and MUST return the same bounded classification and create no runtime when it did not.
- The bounded public failure table MUST remain exhaustive over the proxied runtime failure vocabulary, so a new bounded reconciliation failure category MUST publish exactly one additional row with a safe code and message and no protected value.

### Interfaces
- A top-level navigation-shell boundary validates browser entries, marks same-URL upstream document acquisition, owns one monotonic load generation, and renders fixed loading or error documents without owning runtime processes.
- A `WorkbenchProxyManager` accepts project-library and project-runtime boundaries; the runtime boundary exposes an in-memory exact-running-snapshot ownership check in addition to `start`, plus injected HTTP, WebSocket, clock, timeout, event, and project-attributed audit adapters.
...
- A successful explicit restart leaves prior-generation proxy-owned HTTP streams, handshakes, sockets, and WebSockets unable to communicate with the released runtime, and the post-case audit reports zero surviving proxy-owned resources for that generation.
- Every matrix case has a declared finite timeout, generation input, expected digest or exact outcome, and post-case zero-socket audit. Frame-destination isolation rows MUST establish live source and mismatched-target WebSocket controls, attempt to select the mismatched target socket for uniquely correlated text and binary source frames through the production forwarding boundary, retain send attempts plus both endpoint receipt counters, and observe exact rejection before either endpoint receives those IDs. An independent target-control echo MUST prove the target remains live; rightful-source-only delivery and snapshot-prevalidation rejection are distinct, insufficient rows.
...
- [ADR-260812-browser-navigation-shell](../ADR/ADR-260812-browser-navigation-shell.md)
